...
class Mail:
	...
	def send(self,recv,key):
		try:
			text = f"This is Sravani Mandela your OTP is {key}"
			subject = "Hey , there"
			message = """\
			From: %s
			To: %s
...
class Server:
	def __init__(self):
				...
				self.pub_alice = {}
				self.connections = {}
				with open('C:\\Users\\Sravani Mandela\\Desktop\\CryptMeetUp\\password.txt') as f:
					email,password=f.readline().strip(),f.read().split('\n')[0]
				self.email = Mail(email,password)
				#self.email = Mail(os.getenv("MAIL"),os.getenv("MAIL_PASSWORD"))
				# taking mail and password from system for loggin in
